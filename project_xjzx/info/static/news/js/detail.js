...
$(function(){
    ...
    $.ajax({
        ...
        success: function (resp) {
            if (resp.errno == "0") {
                ...
                // comment_html += '</div>'
                // comment_html += '</div>'
                // comment_html += '<div class="comment_time fl">' + comment.parent.create_time + '</div>'
                //
                // comment_html += '<a href="javascript:;" class="comment_up fr" data-commentid="' + comment.parent.id + '" data-newsid="' + comment.parent.news_id + '">赞</a>'
                // comment_html += '<a href="javascript:;" class="comment_reply fr">回复</a>'
                // comment_html += '<form class="reply_form fl" data-commentid="' + comment.parent.id + '" data-newsid="' + comment.parent.news_id + '">'
                // comment_html += '<textarea class="reply_input"></textarea>'
                // comment_html += '<input type="button" value="回复" class="reply_sub fr">'
                // comment_html += '<input type="reset" name="" value="取消" class="reply_cancel fr">'
                // comment_html += '</form>'
                ...
                // $this.parent().prev('.baoqilai').append($comment_html)
                $this.parent().prev().prev().prev().prev().append($comment_html)
    ...
    var length = $(".comment_list").length
        var length2 = $(".reply_text_con").length
    $(".comment_count").html((length +length2) + "条评论")
}
})
